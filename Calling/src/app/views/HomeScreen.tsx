...
import { ActiveMeetingsDashboard } from './ActiveMeetingsDashboard';
import { RoomLocator, TeamsMeetingLinkLocator } from '@azure/communication-calling';
import { TeamsMeetingIdLocator } from '@azure/communication-calling';
import { getRoomIdFromUrl, getTeamsLinkFromUrl, getMeetingIdFromUrl, getGroupIdFromUrl } from '../utils/AppUtils';
...
export const HomeScreen = (props: HomeScreenProps): JSX.Element => {
  ...
  // Detect call type from URL to set correct initial state
  const isGroupCall = !!getGroupIdFromUrl();
  const isTeamsCallLegacy = !!getTeamsLinkFromUrl() || !!getMeetingIdFromUrl();

  let defaultOptionKey: CallOption = 'ACSCall';
  if (isGroupCall) {
    defaultOptionKey = 'ACSCall';
  } else if (isTeamsCallLegacy) {
    defaultOptionKey = 'TeamsMeeting';
  }

  const headerTitle = props.joiningExistingCall
    ? (isTeamsCallLegacy ? 'Join Teams Call' : 'Join Call')
  ...
  const [chosenCallOption, setChosenCallOption] = useState<ICallChoiceGroupOption>(
    callOptions.find(o => o.key === defaultOptionKey) || callOptions[0]
  );
  ...
  return (
    <Stack className={containerStyle}>
      <Stack horizontalAlign="center" styles={{ root: { height: 'auto', marginBottom: '1rem' } }}>
        <Image alt="Foetron Logo" src={foetronLogo.toString()} styles={{ image: { height: 80 } }} />
      </Stack>
      <Stack
        horizontal
        wrap
      ...
      >
        <Stack styles={{ root: { width: '100%', maxWidth: '20rem' } }}>
          <MeetingHistoryList />
          <ActiveMeetingsDashboard onJoin={(url) => (window.location.href = url)} />
        </Stack>
